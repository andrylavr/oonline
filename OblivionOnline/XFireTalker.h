...
*/
#pragma once
#ifndef _OO_XFIRE_TALKER_H
#define _OO_XFIRE_TALKER_H
#include "main.h"
...
inline void UpdateXFireInfo(const char *Name,unsigned int Level,char* Race, const char *Class,const char *Cell,const char *ServerName,const char *ServerIP,unsigned short Players)
{
	...
	sprintf(Character,"%60s , Level %u %s %s",Name,Level,Race,Class);
	sprintf(VersionString,"OblivionOnline %u.%u %s %s by Julian Bangert aka masterfreek64",MAIN_VERSION,SUB_VERSION,RELEASE_CODENAME,ADDITIONAL_VERSION_COMMENT);
	key[0] = "OblivionOnline"; 
	value[1] = Character; 
	key[1] = "Character"; 
	value[1] = Character; 
	key[2] = "Location"; 
